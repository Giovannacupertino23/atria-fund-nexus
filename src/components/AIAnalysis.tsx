...
const AIAnalysis: React.FC<AIAnalysisProps> = ({
  ...
  return <Card>
      <CardHeader>
        ...
        <CardDescription>Clique em analisar e receba análises contextualizadas baseadas em IA</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <Button onClick={handleSendToWebhook} disabled={isSendingToWebhook} className="w-full">
          {isSendingToWebhook ? <>
        ...
        <div className="space-y-2">
          <Textarea placeholder="A resposta da análise aparecerá aqui..." value={response} onChange={e => setResponse(e.target.value)} className="min-h-[200px]" readOnly={false} />
        </div>
      </CardContent>
    </Card>;
};
